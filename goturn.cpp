...
int main()
{
	...
	VideoCapture cap("http://192.168.2.2:81/videostream.cgi?user=admin&pwd=1004");
	...
	for (int i = 0; i < dataset->getDatasetLength(datasetID); i++)
	{
		...
		else
		{
			...
			//for (int i=0; i<3; i++)
				//cout << (timeStamp[i+1] - timeStamp[i])*1000.0/getTickFrequency() << "ms" << endl;
	...
	cout << "Totap FPS: " <<  dataset->getDatasetLength(datasetID)/((timeStamp[51] - timeStamp[50])/getTickFrequency()) << endl;
